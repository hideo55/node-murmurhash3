...
for(var i = 0; i < testData.length; i++) {
  var val = testData[i];
  exports["testAsyncInterface32_" + i] = function(test, assert) {
    mmh3.murmur32(val[0], function(err, res) {
      assert.equal(res, val[1], 'murmur32-' + i);
      test.finish();
    });
  };
  exports["testAsyncInterface32Hex_" + i] = function(test, assert) {
    mmh3.murmur32Hex(val[0], function(err, res) {
      assert.equal(res, zeroFill(val[1].toString(16), 8), 'murmur32Hex-' + i);
      test.finish();
    });
  };
  exports["testAsyncInterface128_" + i] = function(test, assert) {
    mmh3.murmur128(val[0], function(err, res) {
      assert.deepEqual(res, val[2], 'murmur128-' + i);
      test.finish();
    });
  };
  exports["testAsyncInterface128Hex_" + i] = function(test, assert) {
  ...
  exports["testSyncInterface_" + i] = function(test, assert){
      var res;
